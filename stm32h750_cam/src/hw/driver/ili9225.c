...
static uint16_t _win_w  = ILI9225_LCD_WIDTH;
static uint16_t _win_h  = ILI9225_LCD_HEIGHT;
...
bool ili9225RequestDraw(void)
{
  ...
  ili9225SetAddrWindow(_win_x, _win_y, (_win_x+_win_w)-1, (_win_y+_win_h)-1);

  _writePin(_PIN_DEF_DC, _DEF_HIGH);
  _writePin(_PIN_DEF_CS, _DEF_LOW);

  is_tx_done = false;
  spiDmaTransfer(spi_ch, (uint8_t *)p_frame_buf, _win_w*_win_h, 0);
